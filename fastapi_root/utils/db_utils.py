...
# 替换为你的openGauss连接信息
OPENGAUSS_CONFIG = {
    "host": os.getenv("DB_HOST", "121.36.73.114"),
    "port": os.getenv("DB_PORT", "26000"),
    "user": os.getenv("DB_USER", "zyh"),
    "password": os.getenv("DB_PASSWORD", "Lionel10"),
    "dbname": os.getenv("DB_NAME", "knowledge_db"),
    "sslmode": "require"
}
...
try:
    ...
    print("✅ openGauss 连接池初始化成功")
except Exception as e:
    print(f"❌ openGauss 连接池初始化失败: {e}")
    db_pool = None
...
def test_opengauss_connection():
    ...
    try:
        with get_db_cursor() as cur:
            ...
            result = cur.fetchone()
            print(f"✅ openGauss 连接成功，版本：{result[0]}")
            return True
    except Exception as e:
        print(f"❌ openGauss 连接失败：{str(e)}")
        return False
...
def create_user(username, hashed_password, email=None):
    """创建新用户"""
    sql = "INSERT INTO users (username, password, email) VALUES (%s, %s, %s) RETURNING user_id;"
    with get_db_cursor() as cur:
        try:
...
def get_user_by_username(username):
    """根据用户名获取用户"""
    sql = "SELECT user_id, username, password, email FROM users WHERE username = %s;"
    with get_db_cursor() as cur:
        cur.execute(sql, (username,))
        row = cur.fetchone()
        if row:
